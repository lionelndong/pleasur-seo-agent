...
## Board Override: Feature-Led Mentions, Not Pricing Defaults

This section overrides any looser wording below.

Before annotating the outline, build a Feature Fit Matrix. For each possible product mention, write:

`reader job -> live Pleasur.AI feature/use case -> proof available -> best link -> visual`

Rules:

- If a mention cannot fill this row, cut it.
- Default links are product flows (`/create`, `/generate`) or relevant existing blog walkthroughs.
- `/pricing` is only allowed when the section is about cost, plans, coins, or "is it worth paying?"
- If `/pricing` appears more than once, justify each occurrence in the saved annotated outline.
- Aim for product mentions in 1-3 sections of a normal 6-section article, 3-5 only for a genuinely product-led article.
- Save the matrix near the top of `content-pipeline/4-outlines-annotated/{slug}.md`.
...
- [ ] Feature Fit Matrix exists and every material mention maps reader job -> feature -> proof -> link -> visual
- [ ] `/pricing` is not the default link; it appears only in cost/plan/coin sections or is explicitly justified
...
- The mention only says "we have this feature" and cannot show a screenshot, workflow, live use case, or existing proof
