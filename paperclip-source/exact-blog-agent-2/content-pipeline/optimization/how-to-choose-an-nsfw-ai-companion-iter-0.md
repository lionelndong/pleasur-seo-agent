...
Each row gets its own section below, in table order, so you can go as deep as you want on the criteria that matter most to you. The five-minute checks are written to be run, not admired. You can do most of them inside a free trial before you ever enter a card number.

| Criterion | What "good" looks like | Red flag | How to test it in 5 minutes |
|---|---|---|---|
| Chat realism + memory | Replies stay in character and recall stated details across the conversation | Generic replies, reset behavior, or memory that only works inside one message | Tell it one specific preference, change topics, then ask a related follow-up |
| Customization depth | Personality, appearance, backstory, and scenario controls are explicit and detailed | Only surface avatars or vague "persona" sliders | Build a test companion and count how many controls actually change the first chat |
| Images, voice, and media | Media features match the chat use case and state their limits | Pretty screenshots with unclear quotas, token costs, or safety rules | Find the image and voice rules, then run one low-stakes media request if it's free |
| Adult-content range | Explicit roleplay allowed within clear consent and safety boundaries | "Anything goes," bypass framing, or no prohibited-content language anywhere | Read the content policy, then test one normal adult scenario |
| Pricing transparency | Monthly price, annual discount, token quotas, and add-ons are easy to find | Hero price hides annual-only billing, metered media, or renewal terms | Compare the monthly vs annual price, then find the token-quota line |
| Privacy + data controls | Retention, deletion, training use, and payment discretion are spelled out | "Private" claim with no retention or deletion detail | Search the privacy policy for retention, training, deletion, and opt-out |
| Age verification + clear ToS | Adult access is gated and the rules are easy to inspect | "No ID" or "no signup" sold as the feature | Confirm the age gate and read the adult-use terms |
| Platform fit | The app matches where and how you want to use it | Mobile/web mismatch, weak exports, missing account controls | Check device support, login path, cancellation path, and support docs |
...
A companion that holds its persona is only as good as how much of it you got to design — which is customization, next.
...
Pleasur.AI's [AI Image Generation](https://pleasur.ai/generate) is a useful example of the in-workflow version — style presets and prompt controls, and the ability to generate an image of your companion from inside the chat thread rather than exporting to a different surface. Crushon.AI also offers in-chat image generation, though reviewers consistently rate the output quality lower than dedicated image-gen platforms. That's the criterion working: media that extends the companion you already built, not a disconnected gallery.

Good media is not the same as broad permission, though — the adult-content range is its own test.

[VISUAL:type=screenshot;target=generate;what=Pleasur.AI image-generation surface showing style presets and prompt controls]

## Criterion 4 — Adult-content range within real limits

The right NSFW platform is clear about what explicit content it does support and equally clear about what it won't. "Anything goes" is a red flag, not a feature.
...
Once the boundaries are clear, look at what the app really costs. That's where most of the deception lives.
...
[VISUAL:type=chart;data=research.pricing_observed_2026;style=bar;title=Advertised vs real monthly cost, example NSFW companion apps]
...
The test: open the privacy policy and search it for "retention," "training," "deletion," and "opt-out." If those words aren't there, you already have your answer. For the full vetting process, our [AI Companion Safety Checklist](https://pleasur.ai/blog/ai-companion-safety-checklist) goes deeper than this one criterion can.

Privacy protects your data. Age verification protects the platform's legitimacy — and it reads as a green flag, not friction.

[VISUAL:type=external;url=https://pleasur.ai/blog/ai-companion-safety-checklist;what=Pleasur.AI AI Companion Safety Checklist article header as the deeper safety resource]
...
Learn to tell a real gate from theater. A real one is a hard wall: an explicit 18+ confirmation on entry, then an actual verification step (card check, ID provider, or age-estimation prompt) before the explicit tier unlocks — Replika puts 18+ verification in front of erotic chat rather than just an "I am over 18" tickbox ([AI Companion Guides, 2026](https://aicompanionguides.com/blog/replika-review/)). A fake gate is one dismissible checkbox with nothing behind it. That's why the real gate is a green flag, not friction: an operator willing to lose the impulse signup is one building to survive scrutiny from payment processors, app stores, and regulators — exactly who you want holding your most explicit chats. The fake gate signals the opposite — fastest possible card entry, no one to answer to when something goes wrong.
...
Pleasur.AI gates the adult tier behind an explicit 18+ confirmation on signup, and the subscription step itself — which requires a card before any explicit content unlocks — adds a second hard wall that an anonymous checkbox does not. The test follows directly: confirm a real gate stops you on entry, skim the terms for a prohibited-content section and a cancellation clause, and if "no verification" is anywhere in the pitch, close the tab.
...
| Replika | Strong personality, lighter explicit controls | Pro $19.99/mo for explicit chat ([AI Companion Guides, 2026](https://aicompanionguides.com/blog/replika-review/)) | 18+ verification before explicit tier — a green flag |
...
Choosing an NSFW AI companion was never about trusting a listicle. It's about whether a platform passes the eight criteria that matter for explicit, private, ongoing companion interaction — and you now hold the test for each one.
