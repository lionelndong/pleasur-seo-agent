...
![Flow diagram titled "Why an AI forgets". Left to right: "Your messages" (a stack of chat bubbles) flows into "Context window" (a fixed-size box with older bubbles spilling out the top, labeled "pushed out"), which flows to "AI reply". Below, a second arrow shows "Close app / new session" emptying the box to blank. Clean editorial illustration, white background, sans-serif labels, brand-neutral colors.](images/why-does-my-ai-companion-forget/image-1-flow-diagram-titled-why-an-ai.png)
...
![Labeled illustration of a context window as a desk. A desk surface holds a fixed number of paper sheets labeled "recent messages". An arrow adds a "new message" sheet on the right](images/why-does-my-ai-companion-forget/image-3-labeled-illustration-of-a-cont.png)
...
![Side-by-side comparison. Left panel titled "In-chat overflow": one long scroll of messages with the earliest ones fading out at the top. Right panel titled "Session reset": two separate chat sessions divided by a wall](images/why-does-my-ai-companion-forget/image-4-side-by-side-comparison-left-p.png)
...
Start with the cost. Scaling a window up isn't like adding pages to a notebook — the compute required climbs much faster than the window does. A model holding ten million tokens in mind needs vastly more processing power per reply than one holding 128,000 — because [attention cost scales quadratically with context length](https://mbrenndoerfer.com/writing/quadratic-attention-bottleneck-transformers-long-sequences), so doubling the window roughly quadruples the work. That's the unglamorous reason companies meter and cap memory: somebody pays for every token the model reads. It's also why no app, ours included, is honestly "unlimited."

Then there's the part most people don't expect: even when a fact *is* still inside the window, the model can effectively forget it. There's a well-documented pattern, sometimes called the ["Lost in the Middle" effect](https://arxiv.org/abs/2307.03172), where models pay closest attention to the start and end of a long context and skim what's between — so a detail from the middle of a long chat can be technically present and functionally ignored. Bigger window, same blind spot.
...
![Why a giant context window costs more — relative compute, 128K vs 1M vs 10M tokens](images/why-does-my-ai-companion-forget/chart-5-why-a-giant-context-window-cos.png)

![A long horizontal bar representing a context window. Markers show "high recall" at the far left and far right ends, and a shaded dip labeled "Lost in the Middle — details dropped" across the center. Clean editorial illustration, white background, sans-serif labels.](images/why-does-my-ai-companion-forget/image-6-a-long-horizontal-bar-represen.png)
...
![Flow diagram of a memory workaround. "New chat starts" → "App pulls saved facts/summary from store" → "Facts injected into context window" → "Model replies as if it remembers". A small store icon labeled "saved facts & summaries" feeds the injection step. Clean editorial illustration, white background, sans-serif labels.](images/why-does-my-ai-companion-forget/image-7-flow-diagram-of-a-memory-worka.png)
...
These are opinionated voice, population quantifiers, superlatives, or named-brand mentions — NOT auto-linked. Editor decides per case whether any needs sourcing.

- L29 — "Different models hold different amounts — a few thousand words, or a few hundred thousand" (range generalization, no specific model named; voice).
- L43 — "Many chat models are stateless by default" (population quantifier; broadly true, but no single source — voice).
- L55 — "Platforms tend to *log* nearly everything you say but only *activate* a small slice as usable memory" (population generalization; voice).
- L70 — "no app, ours included, is honestly 'unlimited'" (comparative absolute / brand-honesty framing; voice, not a number).
- L72 — "well-documented pattern" (now backed by the Lost-in-the-Middle citation; verify the adjective still reads fairly).
- L86 — "ChatGPT's opt-in memory saves 'important information'" and "Character.AI's memory is roleplay-specific and tied *per character*" (named-brand mentions describing competitor behavior; not linked per skill rule — editor may add official-doc links if desired).
- L102 — "those are rarer than the marketing implies" (superlative/opinion; voice).
- L113 — "Pick the companion that passes the next-session check, not the one with the best screenshots" (opinion; voice).
- L117 — "82% seven-day retention against a rival's 33%" — must-cite, LINKED to the [OpenMind vs Pleasur.ai comparison](https://pleasur.ai/blog/openmind-ai-vs-pleasurai); the in-line "no published methodology or sample" caveat is intact and load-bearing. Kept (not softened) because the source and caveat both hold.

### Must-cite density

4 distinct must-cite claims (token/word conversion; 10M-vs-128K compute; "Lost in the Middle"; 82%/33% retention) — all 4 linked = **100%** (well above the 60% floor). No Citation gaps block required.
