...
[VISUAL:type=image;sub=concept-illustration;prompt=Clean editorial diagram of a fixed-width frame labeled "context window" sliding right across a long horizontal strip of chat bubbles; the oldest bubbles on the left fall outside the frame and fade out, labeled "dropped". White background, sans-serif labels, no human likeness, brand-neutral colors.;style=illustration;safety=sfw]
...
[VISUAL:type=chart;data=research.context_rot_accuracy;style=line;title=Recall accuracy drops as the context window fills]
...
[VISUAL:type=screenshot;target=create;what=Companion Creator showing where backstory, personality, and preferences are set — the fields that seed the persistent-memory layer;annotate=.backstory-field]
