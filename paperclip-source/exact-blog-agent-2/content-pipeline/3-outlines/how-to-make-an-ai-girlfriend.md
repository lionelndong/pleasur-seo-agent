...
- **Key points:**
  ...
  - **Step 3 — Voice.** Pick a voice profile she'll use when you tap the speaker icon on her replies. On Pleasur.AI this lives inside the chat — voice replies are tap-to-play per message, not a separate mode. A two-way phone-call action is coming this week on the same chat surface.
  ...
  - **What you get when you're done.** A persistent chat with memory across sessions, the ability to generate scene images of her inside the chat via in-conversation image generation, and a voice for every reply. Most apps also let you remix community-shared characters if you don't want to build from scratch.
...
- **Visuals:**
  - Visual 1: {type: action-shot, url: https://pleasur.ai/create, goal: Log into Pleasur.AI with the saved session. Open the Companion Creator at /create. Dismiss any age-verification dialog. Pick the Realistic template. Click through to the personality/archetype step. Capture the screen showing the archetype options and the backstory field., what: Companion Creator personality and backstory step, mid-wizard}
  - Visual 2: {type: table, columns: [App, Body customization, Voice, In-chat image gen, Monthly price (public)]} — five rows: Nomi, Kupid, Candy.ai, RomanticAI, Pleasur.AI. Cells use research-sourced values; "—" when the platform doesn't disclose. Skim-readable, MECE with the DIY-cost table earlier.
...
- **`/product-mentions`:** the natural live-product slots are H2 "The off-the-shelf path" (AI Companion Creator at `pleasur.ai/create`; in-chat AI Image Generation when discussing post-creation scene gen) and the conclusion. The "what breaks" H2 is the right place to tease in-chat Voice Replies and the in-chat Phone Call (both `coming-soon` per brand-config — no separate URLs, framed as in-chat actions only). Do not mention live products inside the DIY H2 — that section would lose its honesty.
- **`/draft`:** the DIY section must be technically correct (Llama-3-8B, LoRA, SDXL/Flux, ElevenLabs/Coqui XTTS). If any model name drifts in drafting, fail the gate. Adult imagery is editor-managed via `pleasur.ai/generate` — no `image` placeholder in this outline is `safety=adult`.
- **`/quality-check`:** the adversarial read should focus on the DIY section (must not sound like marketing for the off-the-shelf path; the honest off-ramp paragraph is doing real work) and the "what breaks" section (must not preach; the Stanford numbers are evidence, not a lecture).
